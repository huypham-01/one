...
import '../data/action_provider.dart';
import '../data/models/device_response_model.dart';

class ApproveAction extends StatefulWidget {
  const ApproveAction({super.key});

  @override
  State<ApproveAction> createState() => _ApproveActionState();
}

class _ApproveActionState extends State<ApproveAction>
  ...
  void initState() {
    ...
    Future.microtask(
      () => Provider.of<ActionProvider>(context, listen: false).fetchActions(),
    );
  }
  ...
  List<ActionItem> getFilteredActions(
    List<ActionItem> actions,
    bool isComplete,
  ) {
    // Lọc theo tab (In progress hoặc Complete)
    List<ActionItem> filtered = actions.where((item) {
      if (isComplete) {
        return item.statusText.toLowerCase().contains('complete');
      } else {
        return !item.statusText.toLowerCase().contains('complete');
      }
    }).toList();
    ...
    if (searchController.text.isNotEmpty) {
      ...
      filtered = filtered.where((item) {
        return item.deviceId.toLowerCase().contains(searchText) ||
            item.title.toLowerCase().contains(searchText) ||
            (item.createdByName?.toLowerCase().contains(searchText) ?? false) ||
            ...
            child: PopupMenuButton<String>(
              ...
              itemBuilder: (context) => [
                ...
                _buildMenuItem(
                  ...
                  text: AppLocalizations.of(context)!.locationManagement,
                  onTap: () {
  ...
  Widget _buildCardList(List<ActionItem> items, bool isComplete) {
    if (items.isEmpty) {
      return Center(
        child: Column(
  ...
  Widget _buildActionCard(ActionItem item, bool isComplete) {
    return Card(
          ...
          children: [
                    ...
                    children: [
                      Row(
                        children: [
                          ...
                          // badge bên phải
                          _buildStatusBadge(item.statusText),
                        ],
                      ),
                      const SizedBox(height: 10),
                      Text(
              ...
              children: [
                ...
                Expanded(
                  child: _buildInfoItem(
                    ...
                    label: AppLocalizations.of(context)!.actionPlan,
              ...
              children: [
                ...
                Expanded(
                  child: _buildInfoItem(
                    ...
                    label: AppLocalizations.of(context)!.plannedcompletiondate,
            ...
            Row(
              ...
              children: [
                GestureDetector(
                  onTap: () {
                    final provider = Provider.of<ActionProvider>(
                      context,
                      listen: false,
                    );
                    provider.fetchPlans(item.id); // load kế hoạch mới nhất
                    showDetailDialog(context, item.id, item.deviceId);
                  },
                  child: Container(
                    padding: const EdgeInsets.symmetric(
                      horizontal: 12,
                ...
                if (isComplete)
                  GestureDetector(
                    onTap: () async {
                      ...
                      await provider.approveAction(item.id, "approved");
                ...
                if (isComplete)
                  GestureDetector(
                    onTap: () async {
                      ...
                      await provider.approveAction(item.id, "rejected");
                ...
                GestureDetector(
                  onTap: () async {
                    ...
                    final confirm = await showDialog<bool>(
                      context: context,
                      barrierDismissible: true, // bấm ra ngoài đóng dialog
                      builder: (context) {
                        return GestureDetector(
                          onTap: () => FocusScope.of(
                            context,
                          ).unfocus(), // ẩn bàn phím khi bấm ra ngoài
                          child: AlertDialog(
                            shape: RoundedRectangleBorder(
                              borderRadius: BorderRadius.circular(12),
                            ),
          ...
          child: Container(
            ...
            child: Consumer<ActionProvider>(
              builder: (context, provider, child) {
                // final action = provider.actions.firstWhere(
                //   (a) => a.id == actionId,
                //   orElse: () => ActionItem(
                //     id: 0,
                //     deviceId: '',
                          ...
                          Row(
                            ...
                            children: [
                              ...
                              Expanded(
                                child: _buildModernInfoRow(
                                  label: AppLocalizations.of(context)!.creator,
                                  value: action.createdByName.toString(),
                                ),
                              ),
                              Expanded(
                                child: _buildModernInfoRow(
                                  label: AppLocalizations.of(context)!.status,
                                  value: action.statusText,
                                ),
                              ),
                            ],
                          ),
