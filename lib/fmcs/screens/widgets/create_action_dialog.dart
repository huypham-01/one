...
class _CreateActionDialogState extends State<CreateActionDialog> {
  ...
  Future<void> _loadDataFromApi() async {
    try {
      final data = await FmcsApiService.fetchActionData();
      setState(() {
        _issueIdController.text = data.issueCode;

        actionPlans = data.planCodes
            .map((code) => ActionPlan(actionId: code))
            .toList();

        int maxNumber = 0;
        for (var code in data.planCodes) {
          if (code.startsWith('AP')) {
            final numPart = int.tryParse(code.substring(2));
            if (numPart != null && numPart > maxNumber) {
              maxNumber = numPart;
            }
          }
        }
        _planCounter = maxNumber;
        _isLoading = false;
      });
    } catch (e) {
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          Expanded(
            child: Text(
              "${AppLocalizations.of(context)!.createaction} - ${widget.device.deviceId}",
  ...
  Widget _buildDialogInfoGrid() {
    ...
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          if (value.temperatureRaw != null) ...[
            _buildSection(AppLocalizations.of(context)!.temperatureLabel, [
              _buildReadOnlyField(
          ...
          if (value.humidityRaw != null) ...[
            _buildSection(AppLocalizations.of(context)!.humidityLabel, [
              _buildReadOnlyField(
                AppLocalizations.of(context)!.actual,
                "${value.humidityRaw}",
              ),
          ...
          if (value.pressureRaw != null)
            _buildSection(AppLocalizations.of(context)!.pressureLabel, [
              _buildReadOnlyField(
                AppLocalizations.of(context)!.actual,
                "${value.pressureRaw}",
              ),
            ...
            return Expanded(
              child: Padding(
                padding: EdgeInsets.only(
                  right: index < fields.length - 1 ? 8 : 0,
                ),
                child: fields[index],
              ),
            );
            ...
            onPressed: () async {
              try {
                final plansJson = actionPlans.map((plan) {
                  return {
                    "plan_text": plan.actionPlan,
                    "est_date": plan.plannedCompletionDate,
                    "owner_name": plan.owner,
                  };
                }).toList();

                final result = await FmcsApiService.createActionWithPlans(
                  ...
                  issueType: _selectedIssueType != '-- Select issue type --'
                      ...
                      : null,
                  metrics: {
                    "temperature": widget.device.temperatureRaw,
                    "humidity": widget.device.humidityRaw,
                    "pressure": widget.device.pressureRaw,
                  },
                  thresholds: {
                    "tempLower": widget.device.tempLower,
                    "tempUpper": widget.device.tempUpper,
                    "humidityLower": widget.device.humidityLower,
                    "humidityUpper": widget.device.humidityUpper,
                    "pressureLower": widget.device.pressureLower,
                    "pressureUpper": widget.device.pressureUpper,
                  },
                  plans: plansJson,
                );
