...
class _IssuesDialogState extends State<IssuesDialog> {
  ...
  void loadPermissions() async {
    canActionUpdate = await PermissionHelper.has("action.update");
    setState(() {});
  }

  Future<void> _loadIssues() async {
    try {
      final issues = await FmcsApiService.fetchIssues(widget.deviceId);
      if (mounted) {
        setState(() {
          _issues = issues;
          _isLoading = false;
  ...
  Future<void> _toggleExpanded(int issueId) async {
    ...
    if (!isExpanded && issue.actionPlans.isEmpty) {
      try {
        final plans = await FmcsApiService.fetchActionPlans(issueId);
        if (mounted) {
          setState(() {
            issue.actionPlans = plans;
            _expandedStates[issueId] = true;
          });
        }
      } catch (e) {
        debugPrint("Error loading action plans: $e");
      }
    } else {
          ...
          SnackBar(
            content: Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    AppLocalizations.of(context)!.completeActionFailed,
                  ),
                ),
              ],
            ),
  ...
  // Reload lại một issue cụ thể thay vì reload toàn bộ
  Future<void> _reloadSingleIssue(int issueId) async {
    try {
      ...
      final updatedIssues = await FmcsApiService.fetchIssues(widget.deviceId);
      ...
      if (isExpanded) {
        final plans = await FmcsApiService.fetchActionPlans(issueId);
        updatedIssue.actionPlans = plans;
      }
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Text(
                          "${AppLocalizations.of(context)!.issue} - ${widget.deviceId}",
                    ...
                    Expanded(
                      child: GestureDetector(
                        ...
                        child: _buildInfoItem(
                          ...
                          "${issue.doneCount}/${issue.planCount}",
  ...
  Widget _buildActionPlansTable(Issue issue) {
    if (issue.actionPlans.isEmpty) {
      return Container(
        padding: const EdgeInsets.all(12),
        decoration: BoxDecoration(
    ...
    return Column(
      children: issue.actionPlans.asMap().entries.map((entry) {
  ...
  Widget buildActionPlanCard(ActionPlan plan, int index, int issueId) {
    final statusColor = _getStatusColor(plan.status);
    final isTodo = plan.status.toLowerCase() == "todo";
