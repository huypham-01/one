...
class MaintenanceDetailScreen extends StatefulWidget {
  ...
  @override
  State<MaintenanceDetailScreen> createState() => _TaskDetailScreenState();
}

class _TaskDetailScreenState extends State<MaintenanceDetailScreen> {
  late Future<List<DetailTaskMaintenance>> futureEquipment;

  @override
  void initState() {
    super.initState();
    futureEquipment = InspectionService.fetchMaintenanceByUuid(widget.uuid);
  }
  ...
  Widget build(BuildContext context) {
    return DefaultTabController(
      ...
      child: Scaffold(
        ...
        appBar: _buildAppBar(),
        body: FutureBuilder<List<DetailTaskMaintenance>>(
  ...
  PreferredSizeWidget _buildAppBar() {
    return AppBar(
      ...
      elevation: 0,
      title: Text(
        'Maintenance Tasks ${widget.equipmentId}',
  ...
  Widget _buildTaskList(
    List<DetailTaskMaintenance> tasks, {
  ...
  Widget _buildTaskCard(DetailTaskMaintenance task, bool isCompleted) {
    return Container(
                      ...
                      Row(
                        children: [
                          ...
                          Container(
                            ...
                            decoration: BoxDecoration(
                              color: getMaintenaceColor(task.type),
                              borderRadius: BorderRadius.circular(6),
                              boxShadow: [
                                BoxShadow(
                                  color: getMaintenaceColor(
                                    task.type,
                                  ).withOpacity(0.3),
                                  blurRadius: 4,
                                  offset: const Offset(0, 2),
  ...
  Widget _buildActionButton(DetailTaskMaintenance task, bool isCompleted) {
    return Container(
      width: 50,
      decoration: BoxDecoration(
        gradient: LinearGradient(
  ...
  void _navigateToDetail(String formId) async {
    final result = await Navigator.of(context).push(
      MaterialPageRoute(
        ...
        builder: (context) =>
            ChecklistDetailPopup(keyW: "maintenance", formId: formId),
      ),
    );
